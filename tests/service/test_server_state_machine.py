...
# from memtable import Memtable, TOMBSTONE, Tombstone
...
#         self.client.put(f"/keys?key={key}&value={value}")

#     @rule(key=keys)
#     def unset(self, key: str):
#         self.model[key] = None
#         self.client.delete(f"/keys?key={key}")

#     @rule(key=keys)
#     def values_agree(self, key: str):
#         res = self.client.get(f"/keys/{key}")
